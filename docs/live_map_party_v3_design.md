...
상태: 테이블 정의 및 REST API 구현 완료. 현재 동작은 `live_map_party_v3_api.md` 참고.
아래 실시간 접속 관리·자동 정리 항목은 후속 WebSocket 단계의 설계다.
...
- 명시적 마지막 퇴장은 즉시 종료한다. 브라우저 종료 감지와 연결 복구 유예 시간,
  빈 방 자동 종료 및 보존 기간 후 기록 삭제는 WebSocket 단계에서 구현한다.

...
-- ISSUE
CREATE TABLE TKL_ISSUE
(
  BOARD_ID TEXT primary key ,
  WRITER TEXT,
  UPDATE_TIME timestamp
);
COMMENT ON COLUMN TKL_ISSUE.BOARD_ID IS '이슈 게시글 ID';
COMMENT ON COLUMN TKL_ISSUE.WRITER IS '이슈 게시글 작성자 이메일';
...
COMMENT ON COLUMN TKL_BOARD_FORUM.DISLIKE_COUNT IS '게시글 싫어요 수';
...
COMMENT ON COLUMN TKL_BOARD_TIP.DISLIKE_COUNT IS '팁 게시글 싫어요 수';
...
COMMENT ON COLUMN TKL_BOARD_PVE.DISLIKE_COUNT IS 'PVE 게시글 싫어요 수';
...
COMMENT ON COLUMN TKL_BOARD_PVP.DISLIKE_COUNT IS 'PVP 게시글 싫어요 수';
...
COMMENT ON COLUMN TKL_BOARD_ARENA.DISLIKE_COUNT IS 'Arena 게시글 싫어요 수';
...
COMMENT ON COLUMN TKL_BOARD_QUESTION.DISLIKE_COUNT IS '질문 게시글 싫어요 수';
...
2. 이모티콘 테이블 만들고 구매 페이지 연동하기 - 디자인 필요
- 아이디, 이모티콘 주소, 가격
...
- 사용자 삭제와 관리자 삭제 문구 분리
- 삭제 버튼 희재 한테 말하기
- 인기 댓글 3개 상단에 먼저 보여주기 - 좋아요 => 댓글 바로가기는 나중에
- 좋아요 싫어요 합친 것 표시
- 공유할 수 있게 주소 복사 추가